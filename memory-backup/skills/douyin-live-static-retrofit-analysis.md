...
8. **Backtrack `Map` / `HashMap` request fields from call sites**
   ...
   - Read ~150-300 lines before the invocation and track the register passed as the map argument.
    - Common patterns observed in Douyin live-room code:
     ...
     - `/webcast/room/enter_backend/` has chunk and non-chunk variants gated by `LIVE_CHUNK_BACKEND_ENABLE`: non-chunk calls `backendRoom(J, biz_params, Map)`, chunk calls `backendRoomV2(x-nx-chunk, J, biz_params, Map)`. In `X/0ZCG.smali`, the `x-nx-chunk` header value can be `"1.0"` when `GlobalContext.isVirtualEnv()` is true, otherwise `"enable"`.
     - `/webcast/room/enter_preload/` / `splitEnterRoom(J, filter_field, Map)` can build a local `LinkedHashMap` in `X/0ZAr.smali` with `latency_ab_tag` from `LIVE_ENTER_LATENCY_IMPROVE_LEVEL.INSTANCE.getValue()` before passing it as the QueryMap.
...
- For signature/header static analysis, do not assume `x-argus`, `x-ladon`, `x-gorgon`, or `x-khronos` will appear as plaintext strings in base smali. In the observed Douyin base tree, direct grep for those header names returned no hits, while adjacent security-chain clues did appear: `x-metasec*` headers, `ISecApi.frameSign(String,int): Map`, `EncryptorUtil.ttEncrypt([B,int): byte[]`, and `MSManagerUtils/MSManager`. When exact header grep is empty, pivot to SecApi / MetaSec / TTNet interceptors / native `.so` boundaries instead of repeating broad string searches.
- In this Hermes container, shell `python` may be unavailable; use `python3` (or `execute_code`) when generating large static scan artifacts from smali.
...
- If dynamic logs show endpoints such as `/webcast/im/fetch/v2/history/`, `/webcast/gift/list/`, `/webcast/gift/effect_game/get/`, `/webcast/gift/play/indicator/`, or `/webcast/lottery/melon/lottery_info/` but exact smali searches for those paths return no hits, do **not** assume the endpoints are absent. In Douyin live these may be issued through dynamic `@Url` Retrofit interfaces, plugin/split dex code, resource-array route/allowlist constants, or runtime path construction.
- For gift endpoints, distinguish **resource/config constants** from request entrypoints. Example observed in this environment: `/webcast/gift/list/` appears only in `res/values/arrays.xml` path arrays, not as a Retrofit declaration or direct call site; record it as a route/allowlist/config clue rather than an API implementation. `/webcast/gift/exhibition/home/` can appear in `smali_classes15/X/0YsP.smali` under a `cmd_show_user_profile -> ["/webcast/user/", "/webcast/gift/exhibition/home/"]` mapping, which looks like profile-card request monitoring/limiting rather than the main gift-list endpoint.
- For IM fetch/history specifically, check dynamic-url APIs before continuing broad endpoint grep. Known candidates in this environment use `@Url String` plus `@HeaderList` and `@QueryMap`:
  - `smali_classes12/com/ss/ugc/live/sdk/platform/network/http/IMessageApi.smali`
  - `smali_classes12/com/bytedance/android/livesdk/message/config/depend/IMessageApi.smali`
  - `smali_classes12/com/ss/ugc/live/sdk/platform/network/http/IUnifyApi.smali`
  - `smali_classes12/com/bytedance/android/livesdk/unify/IUnifyApi.smali`
  - `smali_classes12/com/bytedance/android/livesdk/interactivity/service/uplink/IUplinkApi.smali`
  Typical methods include `doGetAsync(String url, List headers, Map queryMap)`, `doPost(String url, List headers, Map queryMap, Map/TypedOutput body)`, and `doPostAsync(...)`.
- After finding an `@Url` dynamic API, search call sites such as `IMessageApi;->doGetAsync`, `IMessageApi;->doPost`, and `IMessageApi;->doPostAsync`, then backtrack the first argument register to recover path construction and query-map fields. If direct `invoke-interface` searches return no hits in base smali, record that fact: the calls may live in plugin/split dex, proxy/DI generated code, or reflection, so pivot to runtime hooks on the interface methods and lower-level Retrofit/TTNet request objects rather than repeatedly grepping fixed endpoint strings.
- For live backend aggregation, enumerate classes implementing `com/bytedance/android/livesdk/room/api/IBackendHandler` and extract `getPath()` string constants. These are often backend path keys rather than slash URLs. Observed keys include `webcast_lottery_melon_lottery_info` (`X/0ZmU`, gated by `BackendSwitchConfig.getLotteryBackendEnable()`), `webcast_gift_vote_list` (`X/0ZmV`), and `webcast_gift_group_show_users` (`X/0ZbW`). Trace `LiveRoomBackendUtils` calls to `IBackendHandler.getPath()` to map backend keys to final requests/responses.
